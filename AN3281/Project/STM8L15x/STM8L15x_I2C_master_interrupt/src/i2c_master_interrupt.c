...
void I2C_Init(void) {
  ...
  I2C1->OARH |= 0x40;
  I2C1->ITR = 3;                  // enable Event & error interrupts 
  I2C1->CR2 |= 0x04;              // ACK=1, Ack enable
  I2C1->CR1 |= 0x01;              // PE=1
	I2C1->CR1 &= ~0x80;             // Stretch enable
...
u8 I2C_WriteRegister(u16 u16_SlaveAdd, u8 u8_AddType, u8 u8_NoStop, u8 u8_NumByteToWrite, u8 *pu8_DataBuffer) 
{
	...
	if ((I2C1->SR3 & I2C_SR3_BUSY) == I2C_SR3_BUSY)
		return 0 ;
...
{
	// check if communication on going
	if (((I2C1->SR3 & I2C_SR3_BUSY) == I2C_SR3_BUSY) && (u8_NoStop == 0))
		return 0 ;
	// check if STATE MACHINE is in state INI_00
	if (STATE != INI_00)
		return 0 ;
	...
	//generate Start
	 I2C1->CR2 |= 1;
	 STATE = SB_11;
	 I2C1->ITR |= 3;                  // re-enable interrupt
	return 1;
}
...
if ((sr1  & I2C_SR1_RXNE)==I2C_SR1_RXNE)
{
	...
	I2C1->ITR &= ~I2C_ITR_ITBUFEN;  // Disable Buffer interrupts (errata)
}
	...
	if ((sr1 & I2C_SR1_BTF) == I2C_SR1_BTF)
	{
		switch (STATE)
		{
			...
			case BTF_15 : 	
										I2C1->CR2 &= ~I2C_CR2_ACK;                     		// Set NACK (ACK=0)
										*(pu8_DataBuffer_cpy++) = I2C1->DR;                    // Read next data byte
			...
			case BTF_04 : 
								if ((u8_NumByte_cpy) && ((I2C1->SR1 & I2C_SR1_TXE) == I2C_SR1_TXE))
								{
...
*******************************************************************************/
void TIM4_Init (void) {
  CLK->PCKENR1 |= 0x04;               // TIM4 clock enable
