...
#ifdef _RAISONANCE_
void I2C_Slave_check_event(void) interrupt 29 {
#endif
#ifdef _COSMIC_
...
void Init_I2C (void)
{
		...
		I2C1->OARL = (SLAVE_ADDRESS << 1) ;	// set slave address to 0x51 (put 0xA2 for the register dues to7bit address) 
	  ...
	  I2C1->CR2 = 0x04;					// Enable I2C acknowledgement
	  I2C1->FREQR = 16; 					// Set I2C Freq value (16MHz)
	  I2C1->OARL = (SLAVE_ADDRESS & 0xFF) ;							// set slave address LSB 
	  I2C1->OARH = 0xC0 | ((SLAVE_ADDRESS & 0x300) >> 7);	// Set 10bits address mode and address MSB
	#endif
